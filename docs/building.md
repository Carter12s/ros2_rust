...
In this guide, the Iron distribution of ROS 2 is used, but newer distributions can be used by simply replacing 'iron' with the distribution name everywhere.
...
Building `rclrs` requires a standard [ROS 2 installation](https://docs.ros.org/en/iron/Installation.html), and a few extensions.
...
# Install ROS 2 as described in https://docs.ros.org/en/iron/Installation.html
...
docker build -f src/ros2_rust/Dockerfile --build-arg "ROS_DISTRO=iron" -t ros2_rust_dev
```
...
```shell
# Make sure to run this in the workspace directory
vcs import src < src/ros2_rust/ros2_rust_iron.repos
```
...
Sourcing an installation populates a few environment variables such as `$AMENT_PREFIX_PATH`, so if you are not sure, you can check that the `$AMENT_PREFIX_PATH` environment variable contains the ROS 2 installation, e.g. `/opt/ros/iron`.
...
```shell
# You can also do `source /opt/ros/iron/setup.bash` in bash
